...
var AIResumeOptimizer = function () {
  ...
  var handleSendChat = function () {
    ...
    setChatMessages(function (prev) {
      return prev.concat([{ role: "user", content: question }]);
    });
    askAI(question, resumeText, jdText, result)
      .then(function (answer) {
        setChatMessages(function (prev) {
          return prev.concat([{ role: "assistant", content: answer }]);
        });
      })
      ["catch"](function (err) {
  ...
  var quickQuestions = ["这一部分怎么优化？", "为什么匹配度这么低？", "给我一个简历话术改写", "我应该先改哪个部分？"];

  var chatPanelContent = null;
  if (showChat) {
    var chatPanelChildren = [
      React.createElement(
        "div",
        { key: "header", className: "shrink-0 px-4 py-3 border-b dark:border-gray-700 flex items-center justify-between" },
        React.createElement(
          "div",
          { className: "flex items-center gap-2" },
          React.createElement(
            "div",
            { className: "w-7 h-7 bg-gradient-to-br from-blue-500 to-purple-600 rounded-lg flex items-center justify-center" },
        ...
        React.createElement(
          Button,
          { variant: "ghost", size: "sm", onClick: function () { return setShowChat(false); } },
          React.createElement(X, { className: "h-4 w-4" })
        )
      ),
      React.createElement(
        ScrollArea,
        { key: "messages", className: "flex-1 px-4 py-3" },
        React.createElement(
          "div",
          { className: "space-y-4" },
          chatMessages.map(function (msg, index) {
            return React.createElement(
              "div",
              { key: index, className: "flex gap-2 " + (msg.role === "user" ? "flex-row-reverse" : "") },
              React.createElement(
                "div",
                {
                  className:
                    "w-7 h-7 shrink-0 rounded-full flex items-center justify-center text-xs " +
          ...
          isChatting &&
            React.createElement(
              ...
              React.createElement(
                "div",
                { className: "w-7 h-7 shrink-0 rounded-full bg-gradient-to-br from-blue-500 to-purple-600 flex items-center justify-center" },
                React.createElement(Bot, { className: "h-4 w-4 text-white" })
              ),
              React.createElement(
                "div",
                { className: "bg-gray-100 dark:bg-gray-800 rounded-xl px-3 py-2" },
                React.createElement(
                  "div",
    ...
    if (!chatMessages.length && !isChatting) {
      var quickBtns = quickQuestions.map(function (q) {
        return React.createElement(
          "button",
          {
            key: q,
            ...
            className:
              "text-xs bg-gray-100 dark:bg-gray-800 hover:bg-blue-100 dark:hover:bg-blue-900 text-gray-600 dark:text-gray-300 rounded-full px-2 py-1 transition-colors",
          },
          q
        );
      });
      chatPanelChildren.splice(2, 0,
        React.createElement(
          "div",
          { key: "quick", className: "px-4 pb-2" },
          React.createElement("p", { className: "text-xs text-gray-400 mb-2" }, "快捷问题："),
          React.createElement("div", { className: "flex flex-wrap gap-1" }, quickBtns)
        )
      );
    }

    chatPanelChildren.push(
      React.createElement(
        "div",
        { key: "input", className: "shrink-0 px-4 py-3 border-t dark:border-gray-700" },
        React.createElement(
          "div",
          { className: "flex gap-2" },
    ...
    chatPanelContent = React.createElement(
      "div",
      { className: "absolute top-0 right-0 bottom-0 w-[370px] bg-white dark:bg-gray-900 border-l dark:border-gray-700 flex flex-col z-20 shadow-2xl" },
      chatPanelChildren
    );
  }
  ...
  if (!result) {
    var leftPanel = React.createElement(ResumeInput, {
      value: resumeText,
      onChange: setResumeText,
      onFileSelect: handleResumeFileSelect,
    });
    var rightPanel = React.createElement(JDInput, {
      value: jdText,
      onChange: setJdText,
      onImageSelect: handleJdImageSelect,
      onImagePreview: setJdImagePreview,
    });
    var analyzeBtn = React.createElement(
      Button,
      {
        size: "lg",
        onClick: handleAnalyze,
        disabled: isAnalyzing || !canAnalyze,
        className: "px-8 bg-gradient-to-r from-blue-500 to-purple-600 hover:from-blue-600 hover:to-purple-700 text-white shadow-lg shrink-0",
      },
      isAnalyzing
        ? React.createElement(
            React.Fragment,
    ...
    pageContent = React.createElement(
      ...
      React.createElement("div", { className: "flex justify-center pb-4" }, analyzeBtn)
    );
  } else {
    var scoreRing = React.createElement(MatchScoreRing, { score: result.overall_score });
    var dimCards = React.createElement(DimensionCards, { dimensionScores: result.dimension_scores });
    var tabsEl = React.createElement(
      Tabs,
      { defaultValue: "missing", className: "w-full" },
      React.createElement(
        TabsList,
        { className: "grid w-full grid-cols-3" },
        React.createElement(
          TabsTrigger,
          { value: "matched", className: "text-xs" },
          "\u2705 匹配项 (",
        ...
        React.createElement(
          TabsTrigger,
          { value: "missing", className: "text-xs" },
          "\u26a0\ufe0f 缺失项 (",
        ...
        React.createElement(
          TabsTrigger,
          { value: "rewrite", className: "text-xs" },
          "\u270f\ufe0f 改写建议 (",
          result.rewritten_sections ? result.rewritten_sections.length : 0,
          ")"
        )
      ),
      React.createElement(TabsContent, { value: "matched" }, React.createElement(MatchedItems, { items: result.matched_items })),
      React.createElement(TabsContent, { value: "missing" }, React.createElement(MissingItems, { items: result.missing_items })),
      React.createElement(TabsContent, { value: "rewrite" }, React.createElement(RewrittenSections, { sections: result.rewritten_sections }))
    );
    var strategy = React.createElement(StrategyAdvice, { strategy: result.application_strategy, score: result.overall_score });
    var chatEntry = React.createElement(
      "div",
      { className: "bg-gradient-to-r from-blue-50 to-purple-50 dark:from-blue-950/30 dark:to-purple-950/30 rounded-xl p-4 border border-blue-200 dark:border-blue-800" },
      React.createElement(
        "div",
        ...
        React.createElement(
          ...
          React.createElement("p", { className: "text-xs text-gray-500 mt-1" }, '可以追问"这一部分怎么优化"、"为什么这个维度分数这么低"等')
        ),
        React.createElement(
          Button,
          { size: "sm", onClick: function () { return setShowChat(true); }, className: "bg-blue-500 hover:bg-blue-600 text-white" },
    ...
    pageContent = React.createElement(
      "div",
      { className: "space-y-4 max-w-4xl pb-8 pr-[380px]" },
      React.createElement(
        "div",
        { className: "text-xs text-gray-300 dark:text-gray-600 bg-gray-50 dark:bg-gray-900 rounded p-2 font-mono" },
        "\ud83e\udd16 调用模型：MiniMax-Text-01 | 简历：",
        resumeText.length,
        "字 | JD：",
        jdText.length,
        "字"
      ...
      React.createElement(
        ...
        React.createElement("div", { className: "lg:col-span-1" }, scoreRing),
        React.createElement("div", { className: "lg:col-span-3" }, dimCards)
      ),
      tabsEl,
      strategy,
      chatEntry
  ...
  return React.createElement(
    ...
    React.createElement(
      "div",
      { className: "shrink-0 px-4 py-3 border-b bg-white dark:bg-gray-900 dark:border-gray-800 z-10" },
      React.createElement(
        "div",
        { className: "flex items-center justify-between" },
        React.createElement(
